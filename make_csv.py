...
def check_csv():
    file_name = ["regex_result_annoy.csv", "regex_result_disgust.csv", "regex_result_fear.csv", "regex_result_joy.csv",
                 ...
                 "text_data_surprise.csv", "text_fromaudio.csv"]
...
def make_csv():
    ...
    emotions = ["none", "joy", "annoy", "sad", "disgust", "surprise", "fear"]
    label_count = [0] * len(emotions)
    f2 = open('result_sentiment_' + str(label_max) +'_train.csv', 'w', encoding='utf-8-sig', newline='')
    wr2 = csv.writer(f2)
    wr2.writerow(["text", "index"])

    f3 = open('result_sentiment_' + str(label_max) + '_test.csv', 'w', encoding='utf-8-sig', newline='')
    wr3 = csv.writer(f3)
